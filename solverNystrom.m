...
%
%  val = s.getField(z) computes the scattered field of inc{1} at points
%  specified by z.
%
%  val = s.getField(z,index) computes the scattered field of inc{index} at
%  points specified by z. The field from inc{index(k)} is val(:,k).
%
...
classdef solverNystrom < solver
    ...
    methods
        ...
        function val = getField(self,points,index)
            ...
            % store the size of points
            s = size(points);

            % reshape points into a column vector
            points = points(:);

            % compute the far field
            val = self.nystrom_exterior_field(points,index);

            % make val the same size as the original points array
            val = reshape(val,s);
    ...
    methods(Access=private)
        ...
        %-----------------------------------------------------------
        % compute exterior field
        %-----------------------------------------------------------

        function val = nystrom_exterior_field(self,points,index)
            ...
            for j = 1:length(points)
                ...
                double_kernel = 0.25*1i*self.kwave*besselh(1,self.kwave*r).*dp.*jac./r;

                single_kernel = 0.25*1i*besselh(0,self.kwave*r).*jac;
                
                kernel = double_kernel+1i*self.coupling_parameter*single_kernel;
            ...
            % take transpose... do this because it is
            % convenient to compute the exterior field at each point for all
            % right hand sides in one go. This gives the transpose of val
            val = tmp.';
